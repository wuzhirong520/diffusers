...
def main(args):
    ...
    def encode_video(video, traj_video):
        video = video.to(accelerator.device, dtype=vae.dtype).unsqueeze(0)
        video = video.permute(0, 2, 1, 3, 4)  # [B, C, F, H, W]
        image = video[:, :, :1].clone()

        traj_video = traj_video.to(accelerator.device, dtype=vae.dtype).unsqueeze(0)
        traj_video = traj_video.permute(0, 2, 1, 3, 4)  # [B, C, F, H, W]

        latent_dist = vae.encode(video).latent_dist
        traj_latent_dist = vae.encode(traj_video).latent_dist
        ...
        return latent_dist, image_latent_dist, traj_latent_dist
    ...
    def collate_fn(examples):
        ...
        for example in examples:
            ...
            traj_latent_dist = example["instance_traj_video"]

            latent_dist = latent_dist.to(accelerator.device)
            traj_latent_dist = traj_latent_dist.to(accelerator.device)
            ...
            traj_video_latents = traj_latent_dist * vae.config.scaling_factor
            image_latents = image_latent_dist * vae.config.scaling_factor

            video_latents = video_latents.permute(0, 2, 1, 3, 4)
            traj_video_latents = traj_video_latents.permute(0, 2, 1, 3, 4)
            ...
            traj_videos.append(traj_video_latents)

        videos = torch.cat(videos)
        images = torch.cat(images)
        traj_videos = torch.cat(traj_videos)
        videos = videos.to(memory_format=torch.contiguous_format).float()
        images = images.to(memory_format=torch.contiguous_format).float()
        traj_videos = traj_videos.to(memory_format=torch.contiguous_format).float()
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(models_to_accumulate):
                ...
                traj_video_latents = batch["traj_videos"]

                prompt_embeds = prompt_embeds.to(dtype=weight_dtype)
                video_latents = video_latents.to(dtype=weight_dtype)  # [B, F, C, H, W]
                traj_video_latents = traj_video_latents.to(dtype=weight_dtype)  # [B, F, C, H, W]
